import type { ProjectAnalysis, PlanStep, Plan } from '../types.js'
import { generateStructuredOutput } from './openai.js'
import {
  TAILWIND_SYSTEM_PROMPT,
  TAILWIND_USER_PROMPT,
  TAILWIND_FALLBACK_PLAN,
} from './prompts/tailwind.js'
import {
  TOAST_SYSTEM_PROMPT,
  TOAST_USER_PROMPT,
  TOAST_FALLBACK_PLAN,
} from './prompts/toast.js'
import {
  FORMS_SYSTEM_PROMPT,
  FORMS_USER_PROMPT,
  FORMS_FALLBACK_PLAN,
} from './prompts/forms.js'
...
async function createTailwindPlanWithAI(analysis: ProjectAnalysis): Promise<PlanStep[]> {
  try {
    const globalCssFile = analysis.styling.includes('tailwind')
      ? 'src/index.css'
      : 'src/index.css'

    const result = await generateStructuredOutput<{ steps: PlanStep[] }>(
      TAILWIND_SYSTEM_PROMPT,
      TAILWIND_USER_PROMPT({
...
async function createToastPlanWithAI(analysis: ProjectAnalysis): Promise<PlanStep[]> {
  try {
    const result = await generateStructuredOutput<{ steps: PlanStep[] }>(
      TOAST_SYSTEM_PROMPT,
      TOAST_USER_PROMPT({
...
async function createFormsPlanWithAI(analysis: ProjectAnalysis): Promise<PlanStep[]> {
  try {
    const result = await generateStructuredOutput<{ steps: PlanStep[] }>(
      FORMS_SYSTEM_PROMPT,
      FORMS_USER_PROMPT({
...
function createReactQueryPlan(analysis: ProjectAnalysis): PlanStep[] {
  return [
    {
