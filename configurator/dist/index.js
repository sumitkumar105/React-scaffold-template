...
import { generateProject, analyzeProject, CAPABILITIES, } from './lib.js';
...
program
    ...
    if (hasFlags) {
        if (options.tailwind)
            selectedCapabilities.push('tailwind');
        if (options.redux)
            selectedCapabilities.push('redux');
        if (options.reactQuery)
            selectedCapabilities.push('reactQuery');
        if (options.forms)
            selectedCapabilities.push('forms');
    }
    else if (!options.yes) {
        console.log();
