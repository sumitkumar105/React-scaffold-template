 ...
 */
export async function executeReactQuery(projectPath, plan) {
...
`;
    await writeFile(path.join(providersDir, 'QueryProvider.tsx'), queryProviderContent);
    ...
    if (await fileExists(mainPath)) {
        ...
        if (!mainContent.includes('QueryProvider')) {
            ...
            if (mainContent.includes('<BrowserRouter>')) {
                mainContent = mainContent.replace('<BrowserRouter>', '<QueryProvider>\n    <BrowserRouter>');
                mainContent = mainContent.replace('</BrowserRouter>', '</BrowserRouter>\n    </QueryProvider>');
            }
            else if (mainContent.includes('<App />') || mainContent.includes('<App/>')) {
                // If no BrowserRouter, wrap App directly
                mainContent = mainContent.replace(/<App\s*\/>/, '<QueryProvider>\n      <App />\n    </QueryProvider>');
            }
