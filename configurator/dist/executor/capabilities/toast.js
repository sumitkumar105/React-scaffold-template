 ...
 */
export async function executeToast(projectPath, plan) {
    ...
    if (await fileExists(appPath)) {
        ...
        if (!appContent.includes("from 'sonner'")) {
            ...
            if (importMatch) {
                appContent = appContent.replace(importMatch[0], `import { Toaster } from 'sonner'\n${importMatch[0]}`);
            }
            else {
        ...
        if (!appContent.includes('<Toaster')) {
            ...
            appContent = appContent.replace(/<div className="app">/, `<div className="app">\n      <Toaster richColors position="top-right" />`);
        }
